...
int main(int argc, char **argv) {
    ...
    if (!g_dry_run) {
        ...
        if (comm_status != HAL_STATUS_OK) {
            ...
            hal_status_t api_init_status = comm_manager_init_api_server(&api_cfg);
            if (api_init_status != HAL_STATUS_OK) {
                printf("[MAIN] WARNING: comm_manager_init_api_server failed (status=%d), continuing...\n", api_init_status);
            } else {
