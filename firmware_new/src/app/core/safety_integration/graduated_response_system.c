...
hal_status_t graduated_response_get_polling_interval(uint8_t module_addr, uint32_t *polling_interval) {
    ...
    // Apply limits
    if (final_interval < config->min_interval_ms) {
        final_interval = config->min_interval_ms;
    }
    if (final_interval > config->max_interval_ms) {
        final_interval = config->max_interval_ms;
    }
