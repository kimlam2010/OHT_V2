...
void* concurrent_operations_thread(void* arg) {
    ...
    while (performance_test_running && operations_count < 100) { // Reduced from 10000 to 100
        ...
        __sync_fetch_and_add(&performance_metrics.total_operations, 1);
        __sync_fetch_and_add(&performance_metrics.total_response_time_us, response_time_us);
        
        if (response_time_us < 10000) { // < 10ms
            __sync_fetch_and_add(&performance_metrics.successful_operations, 1);
        } else {
            __sync_fetch_and_add(&performance_metrics.failed_operations, 1);
        }
...
void test_basic_performance(void) {
    ...
    printf("Starting basic performance test for %d seconds...\n", TEST_DURATION_SECONDS);
    ...
    printf("Operations per Second: %.2f\n", 
           (double)performance_metrics.total_operations / 
           ((performance_metrics.end_time_us - performance_metrics.start_time_us) / 1000000.0));
    ...
    TEST_ASSERT_LESS_THAN(10000, performance_metrics.avg_response_time_us); // < 10ms average response time
...
// Main function
int main(void) {
    printf("🚀 OHT-50 BASIC PERFORMANCE TEST SUITE\n");
    printf("=====================================\n");
