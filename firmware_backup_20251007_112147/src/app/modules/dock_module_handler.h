...
// RFID Tag Registers (0x7100-0x71FF)
#define DOCK_RFID_TAG_ID_LOW_REG          0x7100  // RFID Tag ID (low word)
#define DOCK_RFID_TAG_ID_HIGH_REG         0x7101  // RFID Tag ID (high word)
#define DOCK_RFID_SIGNAL_STRENGTH_REG     0x7102  // RFID signal strength (0-100)
#define DOCK_RFID_READ_STATUS_REG         0x7103  // RFID read status (0=no tag, 1=tag detected)
#define DOCK_RFID_LAST_READ_TIME_REG      0x7104  // Last RFID read time (ms)

// Accelerometer Data Registers (0x7200-0x72FF)
#define DOCK_ACCEL_X_REG                  0x7200  // Accelerometer X-axis (mg)
#define DOCK_ACCEL_Y_REG                  0x7201  // Accelerometer Y-axis (mg)
#define DOCK_ACCEL_Z_REG                  0x7202  // Accelerometer Z-axis (mg)
#define DOCK_ACCEL_TEMPERATURE_REG        0x7203  // Accelerometer temperature (°C)
#define DOCK_ACCEL_STATUS_REG             0x7204  // Accelerometer status (0=error, 1=ok)

// Dock Proximity Sensors Registers (0x7300-0x73FF)
#define DOCK_PROX_SENSOR_1_REG            0x7300  // Proximity sensor 1 (0=no object, 1=object detected)
#define DOCK_PROX_SENSOR_2_REG            0x7301  // Proximity sensor 2 (0=no object, 1=object detected)
#define DOCK_PROX_SENSOR_1_DISTANCE_REG   0x7302  // Proximity sensor 1 distance (mm)
#define DOCK_PROX_SENSOR_2_DISTANCE_REG   0x7303  // Proximity sensor 2 distance (mm)
#define DOCK_DOCK_CONFIRMED_REG           0x7304  // Dock confirmed status (0=not docked, 1=docked)
...
typedef struct {
    uint32_t tag_id;                // RFID Tag ID (32-bit)
    uint8_t signal_strength;        // Signal strength (0-100)
    uint8_t read_status;            // Read status (0=no tag, 1=tag detected)
    uint32_t last_read_time;        // Last read time (ms)
} dock_rfid_data_t;
...
typedef struct {
    ...
    uint8_t status;                 // Status (0=error, 1=ok)
} dock_accelerometer_data_t;

// Proximity Sensors Data Structure
typedef struct {
    uint8_t sensor_1_status;        // Sensor 1 status (0=no object, 1=object detected)
    uint8_t sensor_2_status;        // Sensor 2 status (0=no object, 1=object detected)
    uint16_t sensor_1_distance;     // Sensor 1 distance (mm)
    uint16_t sensor_2_distance;     // Sensor 2 distance (mm)
    uint8_t dock_confirmed;         // Dock confirmed (0=not docked, 1=docked)
} dock_proximity_data_t;
...
typedef struct {
    ...
    // Real sensor data
    dock_rfid_data_t rfid;          // RFID tag data
    dock_accelerometer_data_t accelerometer; // Accelerometer data
    dock_proximity_data_t proximity; // Proximity sensors data
} dock_module_data_t;
...
/**
 * @brief Get RFID tag ID
 * @param handler Pointer to dock module handler
 * @param tag_id Pointer to store tag ID
 * @return HAL_STATUS_OK on success, error code on failure
 */
hal_status_t dock_module_get_rfid_tag_id(dock_module_handler_t *handler, uint32_t *tag_id);

/**
 * @brief Get RFID signal strength
 * @param handler Pointer to dock module handler
...
/**
 * @brief Get accelerometer temperature
 * @param handler Pointer to dock module handler
...
/**
 * @brief Get accelerometer status
 * @param handler Pointer to dock module handler
...
/**
 * @brief Get proximity sensor 1 status
 * @param handler Pointer to dock module handler
...
/**
 * @brief Get proximity sensor 2 status
 * @param handler Pointer to dock module handler
 * @param status Pointer to store status (0=no object, 1=object detected)
 * @return HAL_STATUS_OK on success, error code on failure
 */
...
/**
 * @brief Get proximity sensor distances
 * @param handler Pointer to dock module handler
...
/**
 * @brief Get dock confirmed status
 * @param handler Pointer to dock module handler
 * @param confirmed Pointer to store dock confirmed status (0=not docked, 1=docked)
 * @return HAL_STATUS_OK on success, error code on failure
 */
