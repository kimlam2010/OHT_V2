...
static void test_dock_module_init(void) {
    ...
    test_assert(status == HAL_STATUS_OK, "Dock module initialization");
    test_assert(handler.initialized == true, "Initialization flag set");
    test_assert(handler.address == 0x05, "Handler address set correctly");
    test_assert(handler.data.status == DOCK_STATUS_IDLE, "Initial status is IDLE");
    test_assert(handler.data.fault_code == DOCK_FAULT_NONE, "Initial fault code is NONE");
    
    // Test double initialization
    status = dock_module_init(&handler, 0x05);
    test_assert(status == HAL_STATUS_OK, "Double initialization check");
    
    // Test deinitialization
    status = dock_module_deinit(&handler);
    test_assert(status == HAL_STATUS_OK, "Dock module deinitialization");
