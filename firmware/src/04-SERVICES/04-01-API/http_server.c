/**
 * @file http_server.c
 * @brief http_server implementation
 * @version 1.0.0
 * @date 2025-01-27
...
hal_status_t http_server_init(http_server_t *server, const http_server_config_t *config) {
    ...
    hal_log_info("[HTTP] Server initialized on port %d", server->config.port);
...
hal_status_t http_server_deinit(http_server_t *server) {
    ...
    hal_log_info("[HTTP] Server deinitialized");
    return HAL_STATUS_OK;
}
...
hal_status_t http_server_start(http_server_t *server) {
    ...
    if (server->server_socket < 0) {
        pthread_mutex_unlock(&server->mutex);
        hal_log_error("[HTTP] Failed to create socket: %s", strerror(errno));
        return HAL_STATUS_ERROR;
    }
    ...
    if (setsockopt(server->server_socket, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) < 0) {
        ...
        server->server_socket = -1;
        pthread_mutex_unlock(&server->mutex);
        hal_log_error("[HTTP] Failed to set socket options: %s", strerror(errno));
        return HAL_STATUS_ERROR;
    }
    ...
    if (bind(server->server_socket, (struct sockaddr*)&server_addr, sizeof(server_addr)) < 0) {
        ...
        server->server_socket = -1;
        pthread_mutex_unlock(&server->mutex);
        hal_log_error("[HTTP] Failed to bind socket: %s", strerror(errno));
        return HAL_STATUS_ERROR;
    }
    ...
    if (listen(server->server_socket, server->config.max_connections) < 0) {
        ...
        server->server_socket = -1;
        pthread_mutex_unlock(&server->mutex);
        hal_log_error("[HTTP] Failed to listen: %s", strerror(errno));
        return HAL_STATUS_ERROR;
    }
    ...
    if (pthread_create(&server->server_thread, NULL, http_server_thread, server) != 0) {
        ...
        server->server_socket = -1;
        pthread_mutex_unlock(&server->mutex);
        hal_log_error("[HTTP] Failed to create server thread");
        return HAL_STATUS_ERROR;
    }
    ...
    hal_log_info("[HTTP] Server started on port %d", server->config.port);
    return HAL_STATUS_OK;
}
...
hal_status_t http_server_stop(http_server_t *server) {
    ...
    hal_log_info("[HTTP] Server stopped");
    return HAL_STATUS_OK;
}
...
hal_status_t http_server_add_route(http_server_t *server, 
    ...
    hal_log_info("[HTTP] Added route: %s %s", http_get_method_string(method), path);
    return HAL_STATUS_OK;
}
...
static void* http_server_thread(void *arg) {
    ...
    hal_log_info("[HTTP] Server thread started");
    ...
    while (!server->should_stop) {
        ...
        if (client_socket < 0) {
            ...
            hal_log_error("[HTTP] Accept failed: %s", strerror(errno));
            continue;
        }
        ...
        if (status != HAL_STATUS_OK) {
            hal_log_error("[HTTP] Connection handling failed: %d", status);
        }

        close(client_socket);
    }

    hal_log_info("[HTTP] Server thread stopped");
    return NULL;
}
...
static hal_status_t http_handle_connection(http_server_t *server, int client_socket) {
    ...
    if (status != HAL_STATUS_OK) {
        hal_log_error("[HTTP] Failed to parse request");
        return status;
    }
    ...
    if (status != HAL_STATUS_OK) {
        hal_log_error("[HTTP] Failed to process request");
