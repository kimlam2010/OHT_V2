    ...
    if (!g_dry_run) {
        ...
        if (registry_get(0x02, &mi) == 0 && mi.status == MODULE_STATUS_ONLINE /* allow unknown type too */) {
            if (mi.type == MODULE_TYPE_POWER || mi.type == MODULE_TYPE_UNKNOWN) {
                if (power_module_init(&power_handler, 0x02) == HAL_STATUS_OK) {
                    // Apply 4S defaults
                    power_module_config_t cfg;
                    (void)power_module_get_config(&power_handler, &cfg);
                    cfg.voltage_max = 17.0f;
                    cfg.voltage_min = 12.0f;
                    cfg.current_max = 10.0f;
                    cfg.temp_max = 60.0f;
                    (void)power_module_set_config(&power_handler, &cfg);
    ...
    while (g_should_run) {
        ...
        if (!g_dry_run && power_handler_initialized) {
            if ((t - last_power_poll_ms) >= 500) {
                if (power_module_read_all_data(&power_handler) == HAL_STATUS_OK) {
                    bool has_alarm = power_module_check_alarms(&power_handler);
                    if (g_debug_mode) {
                        power_module_print_data(&power_handler);
                    }
                    if (has_alarm) {
                        // TODO: hook into safety manager for degrade/stop if critical
                        printf("[POWER][ALARM] %s\n", power_module_get_alarm_description(power_handler.data.alarm_status));
                    }
                }
