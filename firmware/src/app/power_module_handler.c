...
static hal_status_t power_module_read_battery_data(void)
{
    ...
    status = power_module_handler_read_register(POWER_REG_BATTERY_TEMP, &value);
    if (status == HAL_STATUS_OK) {
        power_module_state.data.battery_temp = (float)value / 10.0f;
    }
    ...
    // Read battery health
    status = power_module_handler_read_register(POWER_REG_BATTERY_HEALTH, &value);
    if (status == HAL_STATUS_OK) {
        power_module_state.data.battery_health = (uint8_t)value;
    }
    
    // Read battery cycles
    status = power_module_handler_read_register(POWER_REG_BATTERY_CYCLES, &value);
    if (status == HAL_STATUS_OK) {
        power_module_state.data.battery_cycles = value;
    }
    
    // Read time to empty
    status = power_module_handler_read_register(POWER_REG_TIME_TO_EMPTY, &value);
    if (status == HAL_STATUS_OK) {
        power_module_state.data.time_to_empty = value;
    }
    
    // Read time to full
    status = power_module_handler_read_register(POWER_REG_TIME_TO_FULL, &value);
    if (status == HAL_STATUS_OK) {
        power_module_state.data.time_to_full = value;
    }
...
static hal_status_t power_module_read_charging_data(void)
{
    ...
    // Read charge current setting
    status = power_module_handler_read_register(POWER_REG_CHARGE_CURRENT_SET, &value);
    if (status == HAL_STATUS_OK) {
        power_module_state.data.charge_current_set = (float)value / 10.0f;
    }
    
    // Read charge voltage setting
    status = power_module_handler_read_register(POWER_REG_CHARGE_VOLTAGE_SET, &value);
    if (status == HAL_STATUS_OK) {
        power_module_state.data.charge_voltage_set = (float)value / 10.0f;
    }
    
    // Read charge enable
    status = power_module_handler_read_register(POWER_REG_CHARGE_ENABLE, &value);
    if (status == HAL_STATUS_OK) {
        power_module_state.data.charge_enable = (value != 0);
    }
    
    // Read charge mode
    status = power_module_handler_read_register(POWER_REG_CHARGE_MODE, &value);
    if (status == HAL_STATUS_OK) {
        power_module_state.data.charge_mode = (uint8_t)value;
    }
    
    // Read charge time remaining
    status = power_module_handler_read_register(POWER_REG_CHARGE_TIME_REMAINING, &value);
    if (status == HAL_STATUS_OK) {
        power_module_state.data.charge_time_remaining = value;
    }
...
static hal_status_t power_module_read_fault_status(void)
{
    ...
    // Read fault status
    status = power_module_handler_read_register(POWER_REG_FAULT_STATUS, &value);
    if (status == HAL_STATUS_OK) {
        power_module_state.data.fault_status = value;
    }
    
    // Read overvoltage fault
    status = power_module_handler_read_register(POWER_REG_OVERVOLTAGE_FAULT, &value);
    if (status == HAL_STATUS_OK) {
        power_module_state.data.overvoltage_fault = (value != 0);
    }
    
    // Read overcurrent fault
    status = power_module_handler_read_register(POWER_REG_OVERCURRENT_FAULT, &value);
    if (status == HAL_STATUS_OK) {
        power_module_state.data.overcurrent_fault = (value != 0);
    }
    
    // Read overtemperature fault
    status = power_module_handler_read_register(POWER_REG_OVERTEMP_FAULT, &value);
    if (status == HAL_STATUS_OK) {
        power_module_state.data.overtemp_fault = (value != 0);
    }
    
    // Read short circuit fault
    status = power_module_handler_read_register(POWER_REG_SHORT_CIRCUIT_FAULT, &value);
    if (status == HAL_STATUS_OK) {
        power_module_state.data.short_circuit_fault = (value != 0);
    }
    
    // Read communication fault
    status = power_module_handler_read_register(POWER_REG_COMM_FAULT, &value);
    if (status == HAL_STATUS_OK) {
        power_module_state.data.comm_fault = (value != 0);
    }
