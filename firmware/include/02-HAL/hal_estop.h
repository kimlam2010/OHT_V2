...
// E-Stop GPIO Pin Definitions (Single channel with software redundancy)
...
#define ESTOP_MODULE_TIMEOUT_MS 3000 // Module communication timeout
#define ESTOP_HEARTBEAT_INTERVAL_MS 1000 // Heartbeat check interval
 ...
 * @return HAL status
 */
hal_status_t hal_estop_get_channel_status(bool *status);

/**
 * @brief Test channel
 * @return HAL status
 */
hal_status_t hal_estop_test_channel(void);

// Module Monitoring Functions

/**
 * @brief Check module communication status
 * @param status Pointer to store communication status
 * @return HAL status
 */
hal_status_t hal_estop_check_module_communication(bool *status);

/**
 * @brief Register module heartbeat
 * @param module_id Module ID
 * @return HAL status
 */
hal_status_t hal_estop_register_module_heartbeat(uint32_t module_id);

/**
 * @brief Check module heartbeat
 * @param module_id Module ID
 * @param alive Pointer to store heartbeat status
 * @return HAL status
 */
hal_status_t hal_estop_check_module_heartbeat(uint32_t module_id, bool *alive);

/**
 * @brief Get module timeout count
 * @param count Pointer to store timeout count
 * @return HAL status
 */
hal_status_t hal_estop_get_module_timeout_count(uint32_t *count);

/**
 * @brief Reset module timeout count
 * @return HAL status
 */
hal_status_t hal_estop_reset_module_timeout_count(void);
